 ...
 * which have set the override_redirect flag. Clients are organized in
 * a linked client list on each monitor, the focus history is
...
void configure(Client *c)
{
    ...
    XSendEvent(dpy, c->win, False, StructureNotifyMask,
               (XEvent *)&ce);
}
...
/* there are some broken focus acquiring clients needing extra
 * handling */
...
int getrootptr(int *x, int *y)
{
    ...
    return XQueryPointer(dpy, root, &dummy, &dummy, x, y, &di, &di,
                         &dui);
}
...
void manage(Window w, XWindowAttributes *wa)
{
    ...
    configure(
        c); /* propagates border_width, if size doesn't change */
    updatewindowtype(c);
    updatesizehints(c);
    updatewmhints(c);
    XSelectInput(dpy, w,
...
void movemouse(const Arg *arg)
{
    ...
    do {
        XMaskEvent(
            dpy, MOUSEMASK | ExposureMask | SubstructureRedirectMask,
            &ev);
        switch (ev.type) {
...
void setup(void)
{
    ...
    XChangeProperty(dpy, wmcheckwin, netatom[NetWMCheck], XA_WINDOW,
                    32, PropModeReplace, (unsigned char *)&wmcheckwin,
                    1);
    XChangeProperty(dpy, wmcheckwin, netatom[NetWMName], utf8string,
